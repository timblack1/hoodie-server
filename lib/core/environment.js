...
exports.getConfig = function (platform, env, project_dir, argv) {
  ...
  if (argv.hasOwnProperty('custom-ports')) {
    ...
    if (c_ports.length === 3) {
      cfg.couch.port = c_ports[0];
      cfg.admin_port = c_ports[1];
      cfg.www_port = c_ports[2];
    } else {
      throw new Error('custom-ports option must be contain 3 ports separated ' +
                      'by commas, ie: --custom-ports=3000,3001,3002');
    }
  } else {
  ...
  if (!process.env.COUCH_URL && !argv.hasOwnProperty('custom-ports')) {
