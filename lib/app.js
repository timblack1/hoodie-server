...
exports.stop = function (config, callback) {
  var tasks = [];
  if (config.www_server) {
    tasks.push(config.www_server.close.bind(config.www_server));
  }
  if (config.admin_server) {
    tasks.push(config.admin_server.close.bind(config.admin_server));
  }
  if (config.nodejitsu_server) {
    tasks.push(config.nodejitsu_server.close.bind(config.nodejitsu_server));
  }
  if (config.multicouch) {
    tasks.push(function (cb) {
      couch.stop(config.multicouch, cb);
    });
  }
